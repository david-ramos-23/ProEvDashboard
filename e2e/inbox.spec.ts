...
test.describe('Emails / Inbox', () => {
  ...
  test('Bandeja muestra tabs de dirección', async ({ page }) => {
    // Make sure Bandeja is selected (default)
    await page.locator('button:has-text("Bandeja")').click();
    const tabs = page.locator('[class*="segmentedTabs"]').first();
    await expect(tabs.locator('button:has-text("Recibidos")')).toBeVisible({ timeout: 10000 });
    await expect(tabs.locator('button:has-text("Enviados")')).toBeVisible();
  });
  ...
  test('tab Recibidos activa el botón', async ({ page }) => {
    await page.locator('button:has-text("Bandeja")').click();
    await page.waitForTimeout(1000);
    const tabs = page.locator('[class*="segmentedTabs"]').first();
    await tabs.locator('button:has-text("Recibidos")').click();
    await expect(tabs.locator('button:has-text("Recibidos")')).toHaveClass(/segmentedTabActive/, { timeout: 3000 });
  });

  test('tab Enviados activa el botón', async ({ page }) => {
    await page.locator('button:has-text("Bandeja")').click();
    await page.waitForTimeout(1000);
    const tabs = page.locator('[class*="segmentedTabs"]').first();
    await tabs.locator('button:has-text("Enviados")').click();
    await expect(tabs.locator('button:has-text("Enviados")')).toHaveClass(/segmentedTabActive/, { timeout: 3000 });
  });
